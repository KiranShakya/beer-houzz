...
describe('PunkApiService', () => {
  ...
  it('should get filtered beers from the API', () => {
    const mockFilterOptions: Partial<PunkApiRequestOptions> = { ids: '1|2|3' };

    service.getFilteredBeers(mockFilterOptions).subscribe((beers) => {
      expect(beers).toEqual(mockResponse);
    });

    const request = httpMock.expectOne(
      'https://api.punkapi.com/v2/beers?ids=1%7C2%7C3&per_page=10&page=1'
    );
  ...
  it('should return cached values', () => {
    const mockFilterOptions: Partial<PunkApiRequestOptions> = { abv_gt: 6 };

    // First request will hit the API and cache the response
    service.getFilteredBeers(mockFilterOptions).subscribe((beers) => {
      expect(beers).toEqual(mockResponse);
    });
    const request = httpMock.expectOne(
      'https://api.punkapi.com/v2/beers?abv_gt=6&per_page=10&page=1'
    );
    expect(request.request.method).toBe('GET');
    request.flush(mockResponse);

    // Second request will return the cached response without hitting the API
    service.getFilteredBeers(mockFilterOptions).subscribe((beers) => {
      expect(beers).toEqual(mockResponse);
    });
    httpMock.expectNone(
